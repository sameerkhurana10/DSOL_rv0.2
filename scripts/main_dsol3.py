...
import dsol.Models_dsol3 as Models
import numpy as np
...
def save_classification_prediction(prediction):
    ...
    np.savetxt(report_path, prediction)
...
def main():
    ...
    if dynamic_args['biofeats'] is not None and dynamic_args['protein_seq_feats'] is not None:
        model.fit([x_train, x_train_bio], y_train,
                batch_size=dynamic_args['batch_size'], class_weight = "auto",
        ...
        [acc_val, score_report_val, cm_val, pred_val] = get_classification_performance(best_model,
                                                                                        [x_val, x_val_bio],
                                                                                        y_val)
        [acc_test, score_report_test, cm_test, pred_test] = get_classification_performance(best_model,[x_test,x_test_bio],y_test)
        ...
        [acc_val, score_report_val, cm_val, pred_val] = get_classification_performance(best_model,
                                                                                        x_val,
                                                                                        y_val)
        [acc_test, score_report_test, cm_test, pred_test] = get_classification_performance(best_model,x_test,y_test)
