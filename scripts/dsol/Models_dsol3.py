...
from keras.layers import add, Concatenate, Embedding, Bidirectional, TimeDistributed, concatenate
...
class DeepSol():
    def __init__(self, static_args, dynamic_args):
        ...
        self.num_bio_feats = dynamic_args['num_bio_feats']
        if self.biofeats is not None:
            self.bio_dnn_config = [conf.split(',') for conf in dynamic_args['bio_dnn_config'].split('-')]
            self.bio_dnn_hidden_dims = [f[0] for f in self.bio_dnn_config]
            self.bio_dnn_hidden_drop = [f[1] for f in self.bio_dnn_config]
    ...
    def fetch_model_def(self):
        ...
        if self.protein_seq_feats is not None:
            ...
            # Append biological feats after passing the bio feats through a Simple DNN
            if self.biofeats is not None:
                y = input_bio_feats
                for a in range(len(self.bio_dnn_config)):
                    h_dims = int(self.bio_dnn_hidden_dims[a])
                    h_dropout = float(self.bio_dnn_hidden_drop[a])
                    y = Dense(h_dims)(y)
                    y = Dropout(h_dropout)(y)
                    y = Activation('relu')(y)
